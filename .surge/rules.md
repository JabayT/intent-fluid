...
- NEVER assign a single deliverable with an estimated workload of "large" or "xlarge" to a single agent to complete all at once in the implement or design phase; it must be pre-split into multiple Parts before dispatch, keeping the estimated output volume of each Part within 70% of the agent's single-output limit. This applies equally to design (Step 6 detailed design) and implement phases — do not wait for a timeout to discover the task was too large; pre-split based on `estimated_output_size` proactively.
...
- ALWAYS include an "estimated vs actual iterations" comparison in the retro output (`estimated_iterations` from topology vs `actual_iterations` at convergence), along with a brief list of factors that accelerated or delayed convergence. This data accumulates across tasks for future topology estimation calibration.
